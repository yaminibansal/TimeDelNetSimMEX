		...
		for (int k = 0; k < QueueSize; ++k){
			if (SpikeQueue[k].size() + NAdditionalSpikesNow[k].load() > SpikeQueue[k].capacity()){
				...
				if (SpikeQueue[k].capacity()){
					do{
						temp = (temp + 1) >> 1; // ceil(temp/2)
						IncreaseExponent++;
					} while (temp > SpikeQueue[k].capacity());
					SpikeQueue[k].reserve(SpikeQueue[k].capacity() * (0x0001 << IncreaseExponent));
				}
				else{
					do{
						temp = temp >> 1; // ceil(temp/2)
						IncreaseExponent++;
					} while (temp > 0);
					SpikeQueue[k].reserve(0x0001 << IncreaseExponent);
				}
			}
			CurrentSpikeLoadingInd[k] = SpikeQueue[k].size();
			SpikeQueue[k].resize(SpikeQueue[k].size() + NAdditionalSpikesNow[k]);
		}
